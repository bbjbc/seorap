...
export default async function run({
  ...
  if (!store.items.length) {
    ...
    await store.addImage(icon.resize({ width: 1600, height: 900 }), {
      source: 'seed',
      title: seedText.wide,
    });
    ...
    await store.addText('npx electron-builder --win --x64', {
      source: 'clipboard',
    });
    const link = await store.addText(
      'https://www.electronjs.org/docs/latest/api/clipboard',
      { source: 'clipboard' },
    );
    if (link)
      store.update(link.item.id, {
        linkTitle: 'clipboard | Electron',
        tags: seedText.tags,
      });
    await store.addFile(path.join(ROOT, 'package.json'), { source: 'drop' });
    await store
      .addFile(path.join(ROOT, 'package-lock.json'), { source: 'drop' })
      .catch(() => null);
    const first = store.items[store.items.length - 1];
    if (first)
      store.update(first.id, { pinned: true, tags: [seedText.iconTag] });
    store.flush();
  }
  ...
  await shot('01-board');
  await js(
    `__seorap.setMode('notes'); __seorap.openNote(__seorap.items().find(i => i.type === 'text' && i.note).id);`,
  );
  await sleep(600);
  await shot('02-notes');
  ...
  const en = lang === 'en';
  vault.add({
    name: en ? 'AWS root account' : 'AWS 루트 계정',
    username: 'me@example.com',
    password: 'x',
    url: 'https://console.aws.amazon.com',
    notes: en ? 'MFA on phone' : 'MFA는 폰에',
  });
  vault.add({ name: en ? 'Home Wi-Fi' : '집 와이파이', password: 'y' });
  vault.add({ name: 'OpenAI API key', username: 'sk-...', password: 'z' });
  await js(
    `__seorap.refreshVault().then(() => __seorap.selectSecret(__seorap.vaultEntryIds()[0]))`,
  );
  await sleep(900);
  ...
  await js(
    `__seorap.closeAllModals(); __seorap.openSwitcher(); __seorap.searchSwitcher(${JSON.stringify(en ? 're' : '메')});`,
  );
  await sleep(400);
  await shot('06-switcher');
  await js(
    `__seorap.closeAllModals(); __seorap.openDetail(__seorap.items().find(i => i.type === 'image').id);`,
  );
  await sleep(600);
  ...
  showToast({
    ...
    thumb: thumbPath
      ? nativeImage.createFromPath(thumbPath).resize({ height: 56 }).toDataURL()
      : null,
    duration: 5000,
  });
  await sleep(700);
  fs.writeFileSync(
    path.join(out, '08-toast.png'),
    (await toastWin.webContents.capturePage()).toPNG(),
  );
