...
export const VaultList = ({ query }: { query: string }) => {
  const t = useT();
  const lang = useLang();
  const { entries, id } = useVaultStore(
    useShallow((s) => ({ entries: s.entries, id: s.id })),
  );
    ...
    <div className="notelist" id="vaultList">
      {!list.length ? (
        ...
        list.map((x) => (
          <div
            key={x.id}
            className={`note-item${x.id === id ? ' active' : ''}`}
            data-id={x.id}
            onClick={() => void selectSecret(x.id)}
          >
