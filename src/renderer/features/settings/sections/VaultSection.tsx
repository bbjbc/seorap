...
export const VaultSection = ({
  ...
  return (
    <Section title={t('settings.vault')}>
      <Row label={t('settings.autolock')} htmlFor="optAutoLock">
        <select
          id="optAutoLock"
          value={String(v.autoLockMinutes)}
          onChange={(e) =>
            void saveSettings({
              vault: { autoLockMinutes: Number(e.target.value) },
            })
          }
        >
      ...
      <Row label={t('settings.clip_clear')} htmlFor="optClipClear">
        <select
          id="optClipClear"
          value={String(v.clipboardClearSeconds)}
          onChange={(e) =>
            void saveSettings({
              vault: { clipboardClearSeconds: Number(e.target.value) },
            })
          }
        >
      ...
      <CheckRow
        id="optLockOnHide"
        checked={v.lockOnHide}
        onChange={(c) => void saveSettings({ vault: { lockOnHide: c } })}
      ...
      <ButtonRow>
        <button
          ...
          id="btnChangeMaster"
          onClick={() => void changeMasterPassword()}
        >
