...
export const ShortcutsSection = ({
  ...
  return (
    <Section
      ...
      hint={t('settings.shortcuts_hint')}
    >
      {SHORTCUTS.map((sc) => (
        <div className="row" key={sc.key}>
          <label htmlFor={sc.id}>{t(sc.label)}</label>
          <ShortcutInput
            id={sc.id}
            value={s.shortcuts[sc.key]}
            onChange={(acc) =>
              void saveSettings({ shortcuts: { [sc.key]: acc } })
            }
          />
          <button
            type="button"
            ...
            data-clear={sc.id}
            onClick={() => void saveSettings({ shortcuts: { [sc.key]: '' } })}
          >
            {t('settings.sc_clear')}
          </button>
        </div>
