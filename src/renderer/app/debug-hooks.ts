...
import { editNoteText, moveNote, newNote, openNote } from '../features/notes/actions';
...
export function installDebugHooks(): void {
  window.__seorap = {
    ...
    noteListIds: () => Array.from(document.querySelectorAll('#noteList .note-item')).map((d) => d.getAttribute('data-id') ?? ''),
