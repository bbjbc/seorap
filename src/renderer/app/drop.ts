...
function onDragEnter(e: DragEvent): void {
  ...
  ui.setDropText(
    ui.mode === 'notes' ? t('board.drop_as_note') : t('board.drop_here'),
  );
}
...
function onDrop(e: DragEvent): void {
  ...
  dragDepth = 0;
  useUiStore.getState().setDropText(null);
  if (e.dataTransfer && !isNoteDrag(e.dataTransfer))
...
async function handleDrop(dt: DataTransfer): Promise<void> {
  ...
  if (files.length) {
    ...
    for (const f of files) {
      ...
      if (isTextFile && mode === 'notes')
        bump(await api.addText(await f.text(), { note: true }));
      else if (p) paths.push(p);
      else
        blobs.push({ name: f.name, mime: f.type, data: await f.arrayBuffer() });
    }
    ...
    else if (uri && /^https?:/i.test(uri) && IMAGE_URL_RE.test(uri))
      bump(await api.addUrl(uri));
    else if (uri && /^https?:/i.test(uri))
      bump(await api.addText(uri, { note: false }));
  ...
  if (added || dup) {
    flash(
      added
        ? dup
          ? t('board.saved_n_dup', { n: added, d: dup })
          : t('board.saved_n', { n: added })
        : t('flash.already_saved'),
    );
    if (mode === 'board') {
