...
describe('generatePassword', () => {
  ...
  it('leaves symbols out when asked', () => {
    for (let i = 0; i < 50; i++)
      expect(generatePassword(24, false)).toMatch(/^[a-zA-Z0-9]+$/);
  });
...
describe('Vault', () => {
  ...
  it('reopens from disk with the right password and rejects the wrong one', () => {
    ...
    expect(() => again.unlock('wrong-password-123')).toThrow(
      t('vault.err_wrong'),
    );
    // 실패 직후엔 대기 시간이 걸린다
  ...
  it('backs off exponentially on repeated failures, capped at 30 seconds', {
    timeout: 30_000,
  }, () => {
    vault.setup(MASTER);
    vault.lock('test');
  ...
  it('rejects changePassword with the wrong current password or a weak new one', () => {
    vault.setup(MASTER);
    expect(() =>
      vault.changePassword('nope-nope-nope-1', 'Another-Strong-Passphrase-99'),
    ).toThrow(t('vault.err_wrong_current'));
